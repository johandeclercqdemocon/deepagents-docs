...
EXPECT_FAILURE: set[tuple[str, int]] = set()
...
def check_own_chapter_refs(fails: list[str]) -> int:
    """Prose references like "Deep Agents Ch 12-17" must point at chapters that exist.

    Only this book's own references can be checked -- the other three books are
    separate repositories. Note the emphasis is stripped first: a pattern that
    expects `**Deep Agents**` never matches `**Deep Agents Ch 1-5**`, and silently
    validates nothing.
    """
    existing = {int(p.name[:2]) for p in (ROOT / "chapters").glob("*.md")}
    pattern = re.compile(r"\b(?:Deep Agents|Chapter) ?(?:Ch|Chapter)?s? ?(\d+)(?:\s*[–—-]\s*(\d+))?")
    n = 0
    for md in markdown_files():
        plain = md.read_text().replace("**", "").replace("*", "")
        for m in pattern.finditer(plain):
            first, last = int(m.group(1)), m.group(2)
            for num in range(first, int(last) + 1) if last else [first]:
                n += 1
                if num not in existing:
                    fails.append(
                        f"bad ref     {md.relative_to(ROOT)} -> Deep Agents Ch {num} "
                        f"(this book has 1-{max(existing)})"
                    )
    return n
...
def main() -> int:
    ...
    try:
        ...
        print("deepagents is not importable by this interpreter:\n"
              f"  {sys.executable}\n\n"
    ...
    counts = {
        ...
        "chapter refs": check_own_chapter_refs(fails),
